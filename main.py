...
var_realiz = 350      # realizations of variations models
zel_realiz = 500      # realizations of zeldovich model
fid_realiz = 10000    # realizations of fiducial model
...
for i in cosmological_pars:
    if "Mnu" not in i and "Ob" not in i:
        ...
        if bool_rsd:
            ...
            derivates_pk_rsd[ind]  = (every_mean_poles_rsd[i+"_p"] - every_mean_poles_rsd[i+"_m"]) / (2 * VarCosmoPar['d_'+i] )
    elif "Mnu" in i:
        # # >>> ORDER 1 >>>
        if order_derivate == 1:
            print('first')
...
if bool_rsd:
    ...
    with h5py.File(root_Pk+'fiducial_pk_nbk_rsd.hdf5', 'r') as f:
        for k in f.keys():
            dat = np.array(f[k])
            if   'mono' in k: fiducial_m_pk_rsd.append(dat)
            elif 'quad' in k: fiducial_q_pk_rsd.append(dat)
            elif 'hexa' in k: fiducial_h_pk_rsd.append(dat)
            elif 'nbk_k' in k: k.append(dat)
            elif 'sn'   in k: sn.append(dat)
            else: assert False, f'keyword {k} not found in `if` code'
    ...
    if max_fids < len(fiducial_wst_rsd):
        ...
        sn_rsd = sn_rsd[new_indices] 

    fiducial_poles_rsd = np.concatenate((fiducial_m_pk, fiducial_q_pk, fiducial_h_pk), axis=1)
